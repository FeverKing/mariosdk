...
var (
	...
	frameIgnored   = regexp.MustCompile(`(?)(github.com/kataras/golog)|(palm/common/sdklog/sdklog.go)|(sdklog.go)`)
	ErrUnknowLevel = errors.New("unknown sdklog level")
)
...
func formatter(l *golog.Log, name string) bool {
	...
	// 还比如 hpzm.go 中是直接使用了 sdklog.DefaultLogger 而不是 GetLogger
...
func CheckLogDir(dir string) error {
	if dir == "" {
		...
		testFilepath := path.Join(dir, "test-sdklog-dir.test")
		defer os.Remove(testFilepath)
		return ioutil.WriteFile(testFilepath, []byte("test sdklog file"), 0640)
	}
}

var DefaultLogger = GetLogger("default")

// Print prints a sdklog message without levels and colors.
...
// Println prints a sdklog message without levels and colors.
...
// then it will print the sdklog message too.
